#ifndef CONTACT_JOINT_TRAJECTORY_CONTROLLER_JOINT_TRAJECTORY_SEGMENT_H
#define CONTACT_JOINT_TRAJECTORY_CONTROLLER_JOINT_TRAJECTORY_SEGMENT_H
...
/**
 * \brief Class representing a multi-dimensional quintic spline segment with a
 * start and end time.
 *
 * It additionally allows to construct the segment and its state type from ROS
 * message data.
 *
 * \tparam Segment Segment type. The state type (\p Segment::State) must define
 * a \p Scalar type
 * (\p Segment::State::Scalar), which can be anything convertible to a \p
 * double; and have the following public members: \code std::vector<Scalar>
 * position; std::vector<Scalar> velocity; std::vector<Scalar> acceleration;
 * \endcode
 */
template <class Segment> class ContactJointTrajectorySegment : public Segment {
public:
  typedef typename Segment::Scalar Scalar;
  typedef typename Segment::Time Time;

  typedef realtime_tools::RealtimeServerGoalHandle<
      talos_wbc_controller::FollowContactJointTrajectoryAction>
      RealtimeGoalHandle;
  ...
  struct State : public Segment::State {
     ...
     *
     * \param position_offset Position offset to apply to the data in \p point.
     * This parameter is useful for handling joints that wrap around (ie.
     * continuous), to compensate for multi-turn offsets. If unspecified
     * (empty), zero offsets are applied; if specified, its size must coincide
     * with that of \p point.
     *
     */
    ...
    void
    init(const trajectory_msgs::JointTrajectoryPoint &point,
	 const std::vector<Scalar> &position_offset = std::vector<Scalar>()) {
	...
	throw(invalid_argument("Size mismatch in trajectory point position, "
			       "velocity or acceleration data."));
      }
	...
	const Scalar offset =
	    position_offset.empty() ? 0.0 : position_offset[i];

	if (!point.positions.empty()) {
	  this->position[i] = point.positions[i] + offset;
	}
	if (!point.velocities.empty()) {
	  this->velocity[i] = point.velocities[i];
	}
	if (!point.accelerations.empty()) {
	  this->acceleration[i] = point.accelerations[i];
	}
      }
    }
  };
   ...
   */
  ContactJointTrajectorySegment(const Time &start_time,
				const State &start_state, const Time &end_time,
				const State &end_state)
      : rt_goal_handle_(), tolerances_() {
    Segment::init(start_time, start_state, end_time, end_state);
  }

  /**
   * \brief Construct a segment from start and end points (boundary conditions)
   * specified in ROS message format.
   *
   * \param traj_start_time Time at which the trajectory containing the segment
   * starts. Note that this is \e not the segment start time. \param start_point
   * Start state in ROS message format. \param end_point End state in ROS
   * message format. \param position_offset See \ref
   * ContactJointTrajectorySegment::State.
   *
   * \throw std::invalid_argument If input parameters are inconsistent and a
   * valid segment can't be constructed.
   */
  ContactJointTrajectorySegment(
      const ros::Time &traj_start_time,
	...
	(traj_start_time + start_point.time_from_start).toSec();
    ...
    try {
      ...
      this->init(start_time, start_state, end_time, end_state);
  ...
  void setGoalHandle(RealtimeGoalHandlePtr rt_goal_handle) {
    rt_goal_handle_ = rt_goal_handle;
  }
...
namespace contact {
/**
 * \param prev_position Previous position from which to compute the wraparound
 * offset. \param next_position Next position from which to compute the
 * wraparound offset. \param angle_wraparound Boolean where true value
 * corresponds to a joint that wrap around (ie. is continuous). Offset will be
 * computed only for this joint, otherwise it is set to zero. \return Wraparound
 * offset that should be applied to \p next_position such that no multi-turns
 * are performed when transitioning from \p prev_position. \tparam Scalar Scalar
 * type.
 */
...
Scalar wraparoundJointOffset(const Scalar &prev_position,
  ...
  if (angle_wraparound) {
    Scalar dist =
	angles::shortest_angular_distance(prev_position, next_position);
